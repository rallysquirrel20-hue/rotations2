    ...
    for i, (name, universe, btype) in enumerate(specs, start=1):
        if universe:
            ...
            if is_current:
                ...
                eq = compute_equity_ohlc_cached(all_df, universe, name, slug, btype)
...
def compute_breadth_from_trend(all_df, universe_by_date):
    """Count uptrend vs downtrend stocks per day."""
    ...
    for d in dates:
        ...
        if day_df is None or day_df.empty:
            continue
        day_df = day_df[day_df['Ticker'].isin(universe_by_date[active_key])]
        if day_df.empty:
            continue
        # Trend is float32: 1.0=up, 0.0=down (after dtype optimization)
        trend_vals = day_df['Trend']
        up_count = int((trend_vals == 1.0).sum()) if trend_vals.dtype != object else int((trend_vals == True).sum())
        down_count = int((trend_vals == 0.0).sum()) if trend_vals.dtype != object else int((trend_vals == False).sum())
        ...
        results.append({
            ...
            'Total_Stocks': total,
            'Breadth_Ratio': (up_count - down_count) / total
        })
    return pd.DataFrame(results)


def compute_breadth_from_breakout(all_df, universe_by_date):
    """Count breakout-regime vs breakdown-regime stocks per day."""
    ...
    for d in dates:
        ...
        if active_key is None:
            continue
        current_universe = universe_by_date[active_key]
        day_df = date_groups.get(d)
        if day_df is None or day_df.empty:
            continue
        day_df = day_df[day_df['Ticker'].isin(current_universe)]
        if day_df.empty:
            continue
        bo_count = int(day_df['Is_Breakout_Sequence'].sum())
        bd_count = int((~day_df['Is_Breakout_Sequence']).sum())
        total = bo_count + bd_count
        if total == 0:
            continue
        results.append({
            'Date': d,
            ...
            'BO_Total_Stocks': total,
            'BO_Breadth_Ratio': (bo_count - bd_count) / total
        })
    return pd.DataFrame(results)
...
def compute_equity_ohlc(
    ...
    # Per-ticker returns: O/H/L/C relative to previous Close
    ...
    for label in quarter_labels:
        if label not in universe_by_date or dv_q is None:
            continue
    ...
    # Walk through dates accumulating OHLC equity
    ohlc_rows = []
    ...
    for d in dates:
        ...
        # Vectorized weighted OHLC returns
        ...
        # Enforce OHLC constraint (high >= max, low <= min)
        ...
        # Drift weights for next day (vectorized)
...
def compute_equity_ohlc_cached(all_df, universe_by_date, basket_name, slug, basket_type='sector'):
    ...
    if FORCE_REBUILD_EQUITY_CACHE:
        cached_df, meta = None, None

    if cached_df is None or not _is_equity_cache_valid(meta, data_sig, universe_sig):
        rebuilt_df, state = compute_equity_ohlc(all_df, universe_by_date, return_state=True)
        if rebuilt_df.empty:
            return rebuilt_df
...
def _compute_within_basket_correlation(universe_by_qtr, window=21):
    """Compute rolling within-basket average pairwise correlation as a daily series.

    Returns a DataFrame with columns ['Date', 'Correlation_Pct'] (scaled 0-100).
    Uses stock returns from all_signals_df for the basket's current constituents.
    """
    if 'all_signals_df' not in globals() or all_signals_df.empty:
        return pd.DataFrame(columns=['Date', 'Correlation_Pct'])

    # Build stock returns matrix for this basket's tickers
    current_key = _get_current_quarter_key()
    if current_key is None:
        keys = sorted(universe_by_qtr.keys()) if universe_by_qtr else []
        current_key = keys[-1] if keys else None
    ...
    # Get all tickers across all quarters for full history
    all_tickers = sorted({t for tickers in universe_by_qtr.values() for t in tickers})
    if len(all_tickers) < 2:
        return pd.DataFrame(columns=['Date', 'Correlation_Pct'])

    df = all_signals_df[all_signals_df['Ticker'].isin(all_tickers)][['Date', 'Ticker', 'Close']].copy()
    df['Date'] = pd.to_datetime(df['Date'], errors='coerce').dt.normalize()
    ...
    # Build quarter lookup for time-varying constituents
    ...
    for d_idx in range(window, len(dates)):
        d = dates[d_idx]
        active_key = _find_active_quarter(d, quarter_labels, quarter_ends)
        if active_key is None:
            corr_values.append({'Date': d, 'Correlation_Pct': np.nan})
            continue
        tickers = [t for t in universe_by_qtr.get(active_key, set()) if t in ret.columns]
        if len(tickers) < 2:
            corr_values.append({'Date': d, 'Correlation_Pct': np.nan})
            continue
        window_data = ret.loc[dates[d_idx - window + 1:d_idx + 1], tickers]
        valid_cols = [c for c in window_data.columns if window_data[c].notna().sum() >= min_obs]
        if len(valid_cols) < 2:
            corr_values.append({'Date': d, 'Correlation_Pct': np.nan})
            continue
        ...
        if pairs.size == 0:
            ...
            corr_values.append({'Date': d, 'Correlation_Pct': np.clip(avg_corr * 100, -100, 100)})

    if not corr_values:
        return pd.DataFrame(columns=['Date', 'Correlation_Pct'])
    return pd.DataFrame(corr_values)


def _augment_basket_signals_with_breadth(signals_df, universe_by_qtr):
    b = compute_breadth_from_trend(all_signals_df, universe_by_qtr).copy()
    ...
    bo = compute_breadth_from_breakout(all_signals_df, universe_by_qtr).copy()
    if not bo.empty:
...
def _finalize_basket_signals_output(name, slug, hist_folder, merged_all, data_sig, universe_sig, universe_by_qtr, basket_type='sector'):
    ...
    corr_df = _compute_within_basket_correlation(universe_by_qtr)
    ...
    # Compute and save per-constituent contributions
...
def _compute_and_save_contributions(slug, basket_type, universe_by_qtr):
    ...
    # Walk through dates capturing per-ticker contributions
    contrib_rows = []
    current_weights_series = None
    current_quarter = None
    ...
    for d in dates:
        ...
        if day_df is None or day_df.empty:
            continue
        day_df = day_df[day_df['Ticker'].isin(universe_by_qtr[current_quarter])]
        if day_df.empty:
            continue
        ...
        w = current_weights_series[common]
        rets = day_data.loc[common, 'Ret'].fillna(0.0)
        contributions = w * rets

        for ticker in common:
            contrib_rows.append({
                'Date': d,
                'Ticker': str(ticker),
                'Weight_BOD': float(w[ticker]),
                'Daily_Return': float(rets[ticker]),
                'Contribution': float(contributions[ticker]),
            })
        ...
        total_w = updated.sum()
        if total_w > 0:
            current_weights_series = updated / total_w
        else:
            current_weights_series = updated
...
def process_basket_signals(name, universe_by_qtr, charts_folder, basket_type='sector'):
    ...
    ohlc_df = compute_equity_ohlc_cached(all_signals_df, universe_by_qtr, name, slug, basket_type)
    if ohlc_df.empty:
    ...
    if cache_valid and pd.notna(cached_last):
        ...
        if not appended_ohlc.empty:
            ...
            if appended_rows:
                ...
                return _finalize_basket_signals_output(
