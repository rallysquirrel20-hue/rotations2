...
Update 2026-04-21: `/api/baskets` now supports optional `universe_start` / `universe_end` quarter params for sidebar parity with `/api/baskets/returns`; `_get_valid_industry_slugs()` now invalidates its cache when the quarter changes or `gics_mappings_500.json` is rewritten.
...
| Endpoint | Reads From |
|----------|------------|
| `GET /api/baskets` | Glob `*_signals.parquet` across cache folders. Themes/sectors always included; industries filtered by `_industries_for_quarter_range()` using optional `universe_start` / `universe_end` params (defaults to current quarter). |
...
|----------|----------------|
| `/api/baskets` (sidebar) | **Default**: current-quarter active industries only via `_industries_for_quarter_range(None, None)`. **With `universe_start`/`universe_end` params**: industries active in any quarter within the supplied range. `App.tsx` now passes these params when quarter-mode universe filtering is active. |
